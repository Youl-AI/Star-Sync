...
def get_ai_interpretation(chart_data, user_concern, lang='ko'):
    ...
    * **위험:** (주의해야 할 행성의 각도와 해결책)
    ...
    ### 🗝️ 행운의 솔루션
    * **행운의 색상:** (색상)
    * **럭키 아이템:** "운을 높여줄 구체적인 럭키 아이템을 하나 추천해 줘.
                        [중요 주의사항] 아이템 이름은 반드시 대괄호 두 개로 감싸서 [[아이템명]] 형태로만 작성해.
                        절대 '형식으로 표기하라'는 등의 시스템 지시문을 출력문에 포함하지 마.
                        자연스러운 문장에 아이템만 괄호로 묶어서 말해. (예시: 당신을 위한 행운의 아이템은 [[노이즈 캔슬링 헤드폰]]입니다.)"
