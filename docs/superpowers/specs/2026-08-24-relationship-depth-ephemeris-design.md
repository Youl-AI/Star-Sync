...
- `src/components/synastry/InviteButton.tsx` (신규, 클라이언트): 내 profile로
  `encodeInvite` → 버튼 두 개.
  1. **카카오톡으로 초대** (주 버튼) — 기존 `KakaoShareButton`/`lib/kakao.ts` feed
     패턴 재사용. 카드: 제목 "궁합 초대가 도착했어요", 본문 한 줄 "두 하늘이
     만나는 자리를 봐요", 이미지 `/og/synastry.png`, 링크는 초대 URL.
     (처음 설계는 URL 복사만이었으나 재검토(2026-08-24)에서 뒤집음 — 복사해서
     카톡에 붙이는 것과 노출 정보가 동일한데 마찰만 컸다. 초대의 존재 이유가
     마찰 제거다.)
  2. **링크 복사** (보조) — `navigator.clipboard.writeText` + "복사됐습니다" 2초
     (IcsRow 복사 버튼 패턴). 카톡 밖 채널용.
- 두 버튼 **위에** 고정 동의 문구(항상 노출, 접지 않음) — 어느 버튼이든 누르기
  전에 읽힌다:
...
- **카카오 경유 fragment 보존 실측**(배포 후, 사용자 휴대폰): 카톡 공유 →
  받은 링크를 카톡 인앱 브라우저로 열었을 때 `#i=`가 살아 있는지. 인앱
  브라우저가 fragment를 떨어뜨리면 카카오 버튼의 링크만 쿼리(`?i=`) 방식으로
  전환한다(쿼리는 Web Analytics에 남을 수 있으나 우리 서버 로그는 없고,
  카카오 전송 자체가 이미 같은 정보를 담는다 — 수용 가능한 절충으로 기록).
