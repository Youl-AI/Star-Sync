...
- `src/components/synastry/InviteButton.tsx` (신규, 클라이언트): 내 profile로
  `encodeInvite` → `navigator.clipboard.writeText` → "복사됐습니다" 2초 표시
  (IcsRow의 복사 버튼과 같은 패턴). 카카오 공유가 아닌 **URL 복사만** — 링크에
  개인정보가 담기므로 전파 채널은 사용자가 직접 고르게 한다.
- 버튼 아래 고정 동의 문구(항상 노출, 접지 않음):
...
- 초대 링크의 카카오 공유 버튼(개인정보 담긴 URL의 전파 채널은 사용자가 고른다).
